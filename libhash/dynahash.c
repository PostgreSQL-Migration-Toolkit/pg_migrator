...
hash_create(const char *tabname, long nelem, HASHCTL *info, int flags)
{
	...
	if (!init_htab(hashp, nelem))
			pg_log(PG_FATAL,"failed to initialize hash table \"%s\"", hashp->tabname);
							...
							bool *foundPtr)
{
	...
	switch (action)
	{
		...
		case HASH_ENTER:
			...
			if (hashp->frozen)
				pg_log(PG_FATAL, "cannot insert into frozen hashtable \"%s\"", hashp->tabname);
			...
			if (currBucket == NULL)
			{
				...
				if (hashp->isshared)
					pg_log(PG_FATAL,"%s", "out of shared memory");
				else
					pg_log(PG_FATAL,"%s", "out of memory");
			}
	...
	pg_log(FATAL, "unrecognized hash action code: %d", (int) action);
					pg_log(PG_FATAL,"%s", "out of memory");
					 ...
					 const void *newKeyPtr)
{
	...
	if (hashp->frozen)
					pg_log(PG_FATAL,"%s", "out of memory");
		pg_log(FATAL, "cannot update in frozen hashtable \"%s\"",
			 hashp->tabname);
	...
	if (currBucket == NULL)
					pg_log(PG_FATAL,"%s", "out of memory");
		pg_log(FATAL, "hash_update_hash_key argument is not in hashtable \"%s\"",
			 hashp->tabname);
...
{
	if (hashp->isshared)
		pg_log(FATAL, "cannot freeze shared hashtable \"%s\"", hashp->tabname);
	if (!hashp->frozen && has_seq_scans(hashp))
		pg_log(FATAL, "cannot freeze hashtable \"%s\" because it has active scans",
			 hashp->tabname);
	hashp->frozen = true;
}
...
hash_corrupted(HTAB *hashp)
{
	 ...
	 */
	if (hashp->isshared)
		pg_log(PANIC, "hash table \"%s\" corrupted", hashp->tabname);
	else
		pg_log(FATAL, "hash table \"%s\" corrupted", hashp->tabname);
}
...
register_seq_scan(HTAB *hashp)
{
	if (num_seq_scans >= MAX_SEQ_SCANS)
		pg_log(FATAL, "too many active hash_seq_search scans, cannot start one on \"%s\"",
			 hashp->tabname);
...
deregister_seq_scan(HTAB *hashp)
{
	...
	pg_log(FATAL, "no hash_seq_search scan for hash table \"%s\"",
		 hashp->tabname);
}
...
AtEOXact_HashTables(bool isCommit)
{
	...
	if (isCommit)
	{
		...
		for (i = 0; i < num_seq_scans; i++)
		{
			pg_log(PG_WARNING, "leaked hash_seq_search scan for hash table %p",
				 seq_scan_tables[i]);
		}
	}
	num_seq_scans = 0;
